...
git clone -b development git@github.com:twidpay-global/twid_system.git /twid/deploy/system
kill $SSH_AGENT_PID
sudo rm -f /twid/deploy/system/.env.local

cp -Rv /twid/deploy/system/* /twid/system
#cp /twid/config/env.system.dev /twid/system/.env
mv /twid/config/env.system.dev.tmp-2 /twid/system/.env
...
echo "successfully deployed dev-system";
echo "Date - $(date)"