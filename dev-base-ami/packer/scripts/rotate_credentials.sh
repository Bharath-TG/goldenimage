...
# Secret names
SECRET_DEV="twid_dev/dev"
SECRET_TMP="twid_dev/tmp"
...
rotate_credentials() {
    ...
    dev_secret_json=$(get_secret "$SECRET_DEV")
    ...
    for role in "${!ROLES[@]}"; do
        ...
        admin_user_key="DEV_ADMIN_USERNAME"
        admin_pass_key="DEV_ADMIN_PASSWORD"
        ...
        host=$(echo "$dev_secret_json" | jq -r ".${host_key}")
        db=$(echo "$dev_secret_json" | jq -r ".${db_key}")
        old_user=$(echo "$dev_secret_json" | jq -r ".${user_key}")
        old_pass=$(echo "$dev_secret_json" | jq -r ".${pass_key}")
        ...
        if create_and_verify_user "$host" "$db" "$admin_user" "$admin_pass" "$new_user" "$new_pass" "$privileges"; then
            ...
            tmp_secret_json=$(echo "$tmp_secret_json" | jq --arg k1 "DEV_${user_key}" --arg v1 "$old_user" \
                                                            --arg k2 "DEV_${pass_key}" --arg v2 "$old_pass" \
                                                            '. + {($k1): $v1, ($k2): $v2}')

            # Update dev secret with new credentials
            dev_secret_json=$(echo "$dev_secret_json" | jq --arg uk "$user_key" --arg uv "$new_user" \
    ...
    update_secret "$SECRET_DEV" "$dev_secret_json"
