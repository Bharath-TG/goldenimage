...
git clone -b development git@github.com:twidpay-global/twid_merchant.git /twid/deploy/merchant
kill $SSH_AGENT_PID

sudo rm -f /twid/deploy/merchant/.env.local
cp -Rv /twid/deploy/merchant/* /twid/merchant
# cp /twid/config/env.merchant.dev /twid/merchant/.env
mv /twid/config/env.merchant.dev.tmp-2 /twid/merchant/.env
...
echo "successfully deployed dev-merchant ";
echo "Date - $(date)"
