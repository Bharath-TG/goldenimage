...
TEMPLATE_FILE="/twid/config/env.system.dev.tmp-1"
OUTPUT_FILE="/twid/config/env.system.dev.tmp-2"
SECRET_NAME="twid_dev/dev"
REGION="ap-south-2"
...
if [ -z "$MODE" ] || [ -z "$STAGE_LABEL" ] || [ -z "$VERSION_ID" ]; then
  echo "Usage: $0 <regular|rollback> <AWSCURRENT|AWSPREVIOUS|NA> <version-id|NA>"
  exit 1
fi
