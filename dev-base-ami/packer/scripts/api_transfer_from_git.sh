...
git clone -b development git@github.com:twidpay-global/twid_api.git /twid/deploy/api
kill $SSH_AGENT_PID

sudo rm -f /twid/deploy/api/.env.local
cp -Rv /twid/deploy/api/* /twid/api
# cp /twid/config/env.api.dev /twid/api/.env
mv /twid/config/env.api.dev.tmp-2 /twid/api/.env
...
echo "Successfully deployed dev-api";
echo "Date - $(date)"
